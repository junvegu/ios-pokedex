...
//  
//
//  Created by Junior Quevedo Gutiérrez  on 18/12/23.
//
...
@IBDesignable open class SelectionList: UIControl {
    ...
    open var items: [Area] = [] {
        didSet {
            deselectAll()
            if let first = items.firstIndex(where: {$0.uid == BDRContext.shared.currentArea.value.uid}) {
                selectedIndex = first
            } else {
...
extension SelectionList: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.font = .bederrRegular(15)
