...
extension UIViewController {
    public func showErrorMessage(message: String) {
        BDRToast(message: message, toastType: .error, duration: .short, toastStyle: .style_pale)
    }
    
    public func showSuccessMessage(message: String) {
        BDRToast(message: message, toastType: .success, duration: .short, toastStyle: .style_pale)
    }
    
    public func showWarningMessage(message: String) {
        BDRToast(message: message, toastType: .warning, duration: .short, toastStyle: .style_pale)
    }
    
    public func BDRToast(message: String, toastType: ToastType, duration: ToastDuration? = .short, toastStyle: ToastStyle? = .style_vibrant, toastGravity: ToastGravity? = .top, toastCornerRadius: Int? = 0, pulseEffect: Bool? = true) {
    ...
    public func BDRToastCustomisation(header: String, message: String, headerColor: UIColor, messageColor: UIColor,
...
func toastStyle_pale(message: String, toastType: ToastType, toastGravity: ToastGravity, view: UIView, pulseEffect: Bool) -> MTPale {
    ...
    switch toastGravity {
        case .top: gravity = CGRect(x: 0.0, y: 80.0, width: view.frame.width, height: 83.0);break
        case .centre: gravity = CGRect(x: 0.0, y: ((view.frame.height / 2) - 41) , width: view.frame.width, height: 83.0);break
        case .bottom: gravity = CGRect(x: 0.0, y: view.frame.height - 30.0, width: view.frame.width, height: 83.0);break
    }
